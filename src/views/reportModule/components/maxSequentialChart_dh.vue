...
const { chartObj } = defineProps(["chartObj"]);
...
const initChart = () => {
      ...
      formatter: function (
        ...
        <div><span>最大:</span>${params[1].data}°</div>
          <div><span>时间:</span>${params[0].axisValue}</div>
            ...
            : `
                <div style="color:white">
        <div><span>data:</span>${params[0].data}°</div>
          <div><span>time:</span>${params[0].axisValue}</div>
                </div>
                `;
        return str;
      },
  ...
  const arrowSize = 15;
  const datay = chartObj[2].settlementY.map((item) => {
  ...
  let deviceSettlingOption: EChartsOption = {
    ...
    tooltip: {
      ...
      formatter: function (params) {
        ...
        <div>水平沉降:${params[0].data.value}mm</div>
